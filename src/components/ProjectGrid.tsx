...
function ProjectRow({ project, index }: { project: Project; index: number }) {
  return (
    <div className="grid grid-cols-1 gap-2 border-b border-[var(--rule)] py-5 sm:grid-cols-[7rem_1fr] sm:py-6">
...
function ProjectTierGroup({ tier, list, startIndex }: { tier: ProjectTier; list: Project[]; startIndex: number }) {
  ...
  return (
    <div className={tier === 'practice' ? 'mt-10 sm:mt-12' : 'mt-8 sm:mt-10'}>
      <div className="flex items-baseline gap-2">
        <h3 className="font-mono-tech text-xs tracking-[0.15em] text-[var(--ink)] uppercase">
          {meta.label}
        </h3>
      ...
      <div className="mt-3 border-t border-[var(--rule)]">
        {list.map((project, i) => (
          <ProjectRow key={project.title} project={project} index={startIndex + i} />
        ))}
      </div>
    </div>
  )
